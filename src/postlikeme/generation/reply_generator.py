...
produce style-matched reply candidates from a VoiceProfile and a target tweet.
"""
...
class ReplyGenerator:
    """Generates style-matched replies to tweets using an LLM.
    ...
    max_length:
        Maximum reply character length for validation.
    """
    ...
    async def generate(
        ...
        """Generate reply candidates matching the voice profile.
        ...
        count:
            Number of reply candidates to generate.
        temperature:
        ...
        -------
        list[str]
            Valid generated reply texts (may be fewer than *count* if some
            candidates fail validation).
        """
        if not target_tweet.strip():
            raise ValueError("target_tweet must not be empty.")

        system_prompt = self._prompt_builder.build_system_prompt(
            profile, max_length=self._max_length
        )
        user_prompt = self._prompt_builder.build_reply_prompt(profile, target_tweet)

        logger.info(
            "Generating %d reply candidates for @%s (temp=%.2f)",
            count,
            profile.username,
            temperature,
        )
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.warning("Reply generation attempt %d failed: %s", i + 1, result)
                continue
            if result:
                candidates.append(result)

        logger.info(
            "Generated %d valid reply candidates out of %d attempts for @%s",
            len(candidates),
            count,
            profile.username,
        )
    ...
    async def generate_ranked(
        ...
        """Generate and rank reply candidates by style adherence.
        ...
        """
        candidates = await self.generate(profile, target_tweet, count, temperature)
    ...
    async def _generate_single(
        ...
        """Generate a single reply candidate and post-process it.

        Returns
        -------
        str | None
            The cleaned reply text, or ``None`` if it fails validation.
        """
        ...
        if not cleaned:
            logger.debug("LLM returned empty reply output after cleaning.")
            return None

        if not self._post_processor.validate_length(cleaned, self._max_length):
            logger.debug(
                "Generated reply exceeds %d chars (%d chars): %s...",
                self._max_length,
                len(cleaned),
                cleaned[:80],
            )
            # Try truncation for slightly over-length replies
